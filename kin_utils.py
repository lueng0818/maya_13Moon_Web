...
def get_oracle(kin):
    ...
    return { "destiny": {"s":s, "t":t}, "analog": {"s": (19-s if 19-s>0 else 19-s+20), "t":t}, 
             "antipode": {"s": (s+10 if s+10<=20 else s-10), "t":t}, "occult": {"s": 21-s, "t": (14-t if 14-t>0 else 14-t+13)}, "guide": {"s":s, "t":t} }
...
def get_user_list():
    conn = get_db()
    try: return pd.read_sql("SELECT 姓名, 生日, KIN FROM Users", conn)
