...
def get_base_matrix_data(kin_num):
    ...
    result = {}
    try:
        # 從 Matrix_Data 中查找基本母體矩陣欄位
        row = conn.execute("SELECT 基本母體矩陣_矩陣位置, 基本母體矩陣_八度音符, 基本母體矩陣_對應腦部 FROM Matrix_Data WHERE 基本母體矩陣_KIN = ?", (kin_num,)).fetchone()
        if row:
            result = {
                "BMU_Position": row['基本母體矩陣_矩陣位置'],
                "BMU_Note": row['基本母體矩陣_八度音符'],
                "BMU_Brain": row['基本母體矩陣_對應腦部']
            }
    except: pass
    finally: conn.close()
    return result
