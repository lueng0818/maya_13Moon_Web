...
這是您的 **13 Moon Synchronotron Master System** 的重大升級版本。

**本次更新重點：**

1.  **自訂「今日」日期**：
      * 在側邊欄新增了 **「📅 日期設定」** 區塊。
      * 您現在可以自由選擇任何一天作為「今日」，系統的 **流日能量、國王棋盤、羅盤校準** 都會根據您選擇的日期連動更新（不再強制鎖定為系統今天）。
2.  **通訊錄進階查詢 (篩選器)**：
      * 在選擇使用者（KIN A）時，新增了 **「篩選器」** 功能。
      * 您可以透過 **調性 (Tone)** 或 **圖騰 (Seal)** 來過濾通訊錄名單。例如：快速找出所有「磁性」的朋友，或所有「紅龍」的朋友。
3.  **人員管理升級 (Tab 7)**：
      * 資料庫表格現在會自動顯示每個人的 **調性** 與 **圖騰** 欄位，方便您一覽無遺。
      * 新增了搜尋與篩選功能。

-----

### 💻 主程式 `app.py` (日期自選 + 通訊錄篩選版)
